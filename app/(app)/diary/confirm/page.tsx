...
export default function DiaryConfirmPage() {
  ...
  const canSendToPublicId =
    diary.trim().length > 0 &&
    normalizedFriendId.length > 0 &&
    !isSubmitting;
  ...
  const handleFriendInput = (value: string) => {
    ...
    setRecipient(
      publicUserId.trim()
        ? { type: "friend", publicUserId: publicUserId.trim() }
        : null,
    );
    setError("");
  };
          ...
          <div className="scroll-area">
            ...
            <button
              ...
              className={`stranger-btn ${
                recipient?.type === "stranger" ? "is-selected" : ""
              }`}
              onClick={handleStrangerSend}
              disabled={!diary.trim() || isSubmitting}
            >
              {isSubmitting && recipient?.type === "stranger"
                ? "送信中..."
                : "初めて出会う誰かに"}
            </button>
