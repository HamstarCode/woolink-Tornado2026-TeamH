...
function CallTimeContent() {
  ...
  const handleTouchMove = (e: React.TouchEvent) => {
    ...
    if (target) {
      handleEnter(Number(target.dataset.index));
    }
  };
  ...
  const rangeStart = hasSelection
    ? formatSlotTime(Math.min(...selectedIndices))
    : null;
  const rangeEnd = hasSelection
    ? formatSlotTime(Math.max(...selectedIndices) + 1)
    : null;
      ...
      <div className="time-phone">
        <section className="time-content">
            ...
            {who === "friend" ? (
              <div className="time-selected-contact">
                ...
                <span className="time-contact-main">
                  ...
                  <span className="time-contact-tag">🌙 サンプル</span>
                </span>
                <span
                  className="time-radio-dot is-checked"
                  aria-hidden="true"
                />
              </div>
            ) : (
              <div className="time-selected-contact">
                ...
                <span className="time-contact-main">
                  <span className="time-contact-name">だれかと</span>
                </span>
                <span
                  className="time-radio-dot is-checked"
                  aria-hidden="true"
          ...
          <div className="time-footer-actions">
            <Link
              className={`time-next-btn${
                hasSelection ? "" : " is-disabled"
              }`}
              href={
                hasSelection
